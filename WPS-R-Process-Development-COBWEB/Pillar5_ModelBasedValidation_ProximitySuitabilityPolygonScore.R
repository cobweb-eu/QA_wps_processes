...
######## function used   
pillar5.ProximitySuitabilityScore <-function(Obs.i,Mod,bufferS, ObsMetaQ=NULL,ModMetaQ=NULL,VolMetaQ=NULL){
  ...
  if(ObsMetaQ[,"DQ_04"]==0.5)ObsMetaQ[,"DQ_04"]=score	
  else ObsMetaQ[,"DQ_04"]=max(ObsMetaQ[,"DQ_04"],score) #DQ_04 
  ## DQ_AbsoluteExternalPositionalAccuracy # DQ_14
  # ObsMetaQ[3] must be already in
  ## DQ_RelativeInternalPositionalAccuracy # DQ_16
  
  if(any(ModMetaQ[,"DQ_16"]!=888)) {    
...
testInit <-function(){
  ...
  #inputModData<<-"029f1c56-da19-453e-80ed-2536f16d0008.shp"
	...
	ObsMeta<<-NULL
	VolMeta<<-NULL
	BufferSizeProx<<-300 # as a test with the sigmoid distance weighting auth at 20% of 800=160 still have a full weigth 
	sFUN<<-"max"
	AttrQuanti<<-TRUE
...
LesA=gBinarySTRtreeQuery(geometry(Mod),RectObs)[[1]]
Mod=Mod[LesA,]  #eg pour Woodland on passe de 58480 à 618 useful
...
if(!is.null(ModMeta) && ModMeta == Moddsn)ModMetaQ=Mod@data



ObsMetaQ=GetSetMetaQ(ObsMetaQ,listQ=c(1,4,14,16),Idrecords= Obs@data[,UUIDFieldName] )
ModMetaQ=GetSetMetaQ(ModMetaQ,listQ=c(14,16,17,18), Idrecords = Mod@data[,ModUUIDFieldName],scope='feature')
...
for (i in 1:dim(Obs@data)[1]){
  ...
  if(!is.null(mod.i)) {		
    Res=pillar5.ProximitySuitabilityScore(Obs[i,],Mod[mod.i,],bufferS, ObsMetaQ[i,],ModMetaQ[mod.i,],VolMetaQ[i,])
    ObsMetaQ[i,]=Res$ObsMetaQ ; ModMetaQ[mod.i,]=Res$ModMetaQ ; VolMetaQ[i,]=Res$VolMetaQ
  }
  else{
    ObsMetaQ[i,"DQ_04"]=0.3333	
  }
...
if(FALSE){ # at the moment as there is no GeoVIQUA service implemented GVQ like are put with the rest
  ...
  if(fullQualityNames)colnames(ModMetaQ)=DQlookup[DQlookup[,1]==colnames(ModMetaQ),2]
  colnames(ModMetaQ)=paste(substring(inputModData,1,7),colnames(ModMetaQ),sep="_")
  Mod@data=cbind(Mod@data,ModMetaQ)	
}
#
...
#out processing
UpdatedObs=paste0(layername, "_outP5PSP.shp")
